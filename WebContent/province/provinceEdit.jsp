...
<body>
	<form action="/province/provinceSave" method="post">
		<input type="hidden" name="id" value='<c:out value="${province.id}" />'/>
		<input type="hidden" name="cid" value='<c:out value="${province.cid}" />'/>
		<table>
				<tr>
					<td>省份 ：</td>
					<td><input type="text" name="name" value='<c:out value="${pname}" />'></td>
				</tr>
				<tr>
					<td>累计确诊：</td>
